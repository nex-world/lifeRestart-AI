...
type ConditionArray = Array<string | ConditionArray>;

/**
 * Parses a condition string into a nested array structure
 */
function parseCondition(condition: string): ConditionArray {
  const conditions: ConditionArray = [];
  const length = condition.length;
  const stack: ConditionArray[] = [];
...
/**
 * Checks if property matches the given condition
 */
function checkCondition(property: PropertyMap, condition: string): boolean {
  const conditions = parseCondition(condition);
  return checkParsedConditions(property, conditions);
}

/**
 * Checks a parsed condition array against property values
 */
function checkParsedConditions(property: PropertyMap, conditions: string | ConditionArray): boolean {
...
/**
 * Checks if a property satisfies a specific condition
 */
...
/**
 * Extracts the maximum number of triggers from a condition
 */
function extractMaxTriggers(condition: string): number {
  ...
  const ageList = matchObject[1].split(",");
